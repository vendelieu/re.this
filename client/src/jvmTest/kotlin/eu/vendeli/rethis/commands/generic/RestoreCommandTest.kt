...
class RestoreCommandTest : ReThisTestCtx() {
    ...
    suspend fun `test RESTORE command without options`() {
        ...
        client.del("testKey1")
        client.restore("testKey1", 0L, keyDump.toByteArray()) shouldBe true
    }
    ...
    suspend fun `test RESTORE command with REPLACE option`() {
        ...
        client.restore("testKey2", 10L, keyDump.toByteArray(), RestoreOption.REPLACE) shouldBe true
    }
    ...
    suspend fun `test RESTORE command with ABSTTL option`() {
        ...
        client.del("testKey3")
        client.restore("testKey3", 10L, keyDump.toByteArray(), RestoreOption.ABSTTL) shouldBe true
    }
    ...
    suspend fun `test RESTORE command with IDLETIME option`() {
        ...
        client.del("testKey4")
        client.restore("testKey4", 10L, keyDump.toByteArray(), RestoreOption.IdleTime(10.seconds)) shouldBe true
    }
    ...
    suspend fun `test RESTORE command with FREQ option`() {
        ...
        client.del("testKey5")
        client.restore("testKey5", 10L, keyDump.toByteArray(), RestoreOption.Frequency(10L)) shouldBe true
    }
}
