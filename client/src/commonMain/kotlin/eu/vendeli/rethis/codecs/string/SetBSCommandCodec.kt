...
public object SetBSCommandCodec {
    ...
    public suspend fun encode(
        ...
        size += 1
        buffer.writeByteStringArg(value, charset, )
        options.forEach { it0 ->
            when (it0) {
                is GET ->  {
                    size += 1
    ...
    public suspend fun decode(input: Buffer, charset: Charset): ByteString? {
        val code = input.parseCode(RespCode.BULK)
        return when(code) {
            RespCode.BULK -> {
                BulkByteStringDecoder.decodeNullable(input, charset, code)
            }
            RespCode.SIMPLE_STRING -> {
                SimpleStringDecoder.decode(input, charset, code).encodeToByteString()
            }
            RespCode.NULL -> {
                null
            }
