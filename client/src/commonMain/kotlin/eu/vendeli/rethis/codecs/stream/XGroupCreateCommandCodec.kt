...
import eu.vendeli.rethis.api.spec.common.request.stream.XId.Id
import eu.vendeli.rethis.api.spec.common.request.stream.XId.LastEntry
...
public object XGroupCreateCommandCodec {
    ...
    public suspend fun encode(
        ...
        mkstream?.let { it0 ->
            if(it0) {
                size += 1
                buffer.writeStringArg("MKSTREAM", charset)
            }
        }
        entriesread?.let { it1 ->
            size += 1
    ...
    public suspend inline fun encodeWithSlot(
        ...
        val request = encode(charset, key = key, group = group, idSelector = idSelector, mkstream = mkstream, entriesread = entriesread)
