...
import eu.vendeli.rethis.shared.decoders.aggregate.MapByteArrayDecoder
...
public object HGetAllBACommandCodec {
    ...
    public suspend fun decode(input: Buffer, charset: Charset): Map<String, ByteArray?> {
        val code = input.parseCode(RespCode.ARRAY)
        return when(code) {
            RespCode.ARRAY -> {
                MapByteArrayDecoder.decodeNullable(input, charset, code)
            }
            RespCode.MAP -> {
                MapByteArrayDecoder.decodeNullable(input, charset, code)
            }
