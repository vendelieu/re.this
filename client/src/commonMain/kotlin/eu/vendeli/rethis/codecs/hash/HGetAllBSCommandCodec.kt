...
import eu.vendeli.rethis.shared.decoders.aggregate.MapByteStringDecoder
...
public object HGetAllBSCommandCodec {
    ...
    public suspend fun decode(input: Buffer, charset: Charset): Map<String, ByteString?> {
        val code = input.parseCode(RespCode.ARRAY)
        return when(code) {
            RespCode.ARRAY -> {
                MapByteStringDecoder.decodeNullable(input, charset, code)
            }
            RespCode.MAP -> {
                MapByteStringDecoder.decodeNullable(input, charset, code)
            }
