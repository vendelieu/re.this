...
class SortRoCommandTest : ReThisTestCtx() {
    @Test
    fun `test SORT_RO command without options`(): Unit = runBlocking {
        client.rPush("testKey1", "1.0", "1.2", "2.0").shouldNotBeNull()
        client.sortRo("testKey1") shouldBe listOf("1.0", "1.2", "2.0")
    }

    @Test
    fun `test SORT_RO command with BY option`(): Unit = runBlocking {
    ...
    @Test
    fun `test SORT_RO command with LIMIT option`(): Unit = runBlocking {
